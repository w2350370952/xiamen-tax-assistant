...
function extractPeriodsFromText(items) {
  const text = items.map((item) => clean(item.str)).join("").replaceAll("：", ":").replace(/[—–]/g, "-");
  const periods = { 上午: ["08:30", "11:30"], 下午: ["14:30", "17:30"], 晚上: ["18:30", "21:30"] };
  for (const name of Object.keys(periods)) {
    const match = text.match(new RegExp(`${name}:?(\\d{1,2}):(\\d{2})-(\\d{1,2}):(\\d{2})`));
    if (match) periods[name] = [`${match[1].padStart(2, "0")}:${match[2]}`, `${match[3].padStart(2, "0")}:${match[4]}`];
  }
  return periods;
}
...
export function parseExtractedPages(pages) {
  ...
  for (const page of pages) {
    ...
    dates.forEach((dateItem, index) => {
      ...
      periodItems.forEach((periodItem) => {
        ...
        const course = {
          id: `${date}-${startTime}-${courses.length + 1}`,
          date, weekday, period,
          time: period,
  ...
  if (!courses.length) warnings.push("未识别到课程，请确认 PDF 为学校课程总表，或先进行 OCR 处理");
