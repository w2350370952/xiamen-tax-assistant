...
export default function App(){
  ...
  return <div className="shell">
    ...
    <main>
      ...
      {tab==="week"&&<section className="page"><Header eyebrow="WEEKLY SCHEDULE" title="周课表" sub={`${weekDays[0].getMonth()+1}月${weekDays[0].getDate()}日 – ${weekDays[6].getMonth()+1}月${weekDays[6].getDate()}日`} actions={<Pager prev={()=>setWeekAnchor(addDays(weekAnchor,-7))} current={()=>setWeekAnchor(initialFocus)} next={()=>setWeekAnchor(addDays(weekAnchor,7))}/>}/><div className="week-scroll"><div className="week-grid"><div className="corner">时段</div>{weekDays.map((d,i)=><div className={`day-head ${iso(d)===today?"today":""}`} key={iso(d)}><span>{weekNames[i]}</span><b>{d.getDate()}</b></div>)}{["上午","下午"].map(period=><WeekRow key={period} period={period} days={weekDays} courses={courses} select={setSelected}/>)}</div></div><Legend/></section>}
      {tab==="month"&&<section className="page"><Header eyebrow="MONTHLY CALENDAR" title={`${monthStart.getFullYear()}年${monthStart.getMonth()+1}月`} sub="按月查看全部课程安排" actions={<Pager prev={()=>{const d=addMonths(monthAnchor,-1);setMonthAnchor(d);setSelectedDay(iso(d));}} current={()=>{setMonthAnchor(firstMonth(initialFocus));setSelectedDay(iso(initialFocus));}} next={()=>{const d=addMonths(monthAnchor,1);setMonthAnchor(d);setSelectedDay(iso(d));}}/>}/><div className="month-calendar"><div className="weekdays">{weekNames.map(x=><span key={x}>{x.replace("星期","周")}</span>)}</div><div className="month-grid">{monthDays.map(d=>{const key=iso(d), dayCourses=courses.filter(c=>c.date===key);return <div key={key} className={`month-cell ${d.getMonth()!==monthStart.getMonth()?"outside":""} ${key===today?"today":""} ${key===selectedDay?"selected":""}`}><button className="date-btn" onClick={()=>setSelectedDay(key)}><b>{d.getDate()}</b>{dayCourses.length>0&&<small>{dayCourses.length}节</small>}</button><div className="month-courses">{dayCourses.map(c=>{const p=palette[c.course_type]||palette.other;return <button aria-label={c.course_name} title={c.course_name} key={c.id} style={{"--color":p.color,"--soft":p.soft}} onClick={()=>{setSelectedDay(key);setSelected(c)}}><i/><span>{c.course_name}</span></button>})}</div></div>})}</div></div><Panel title={`${selectedDay.replaceAll("-",".")} 课程`} count={courses.filter(c=>c.date===selectedDay).length}>{courses.some(c=>c.date===selectedDay)?<div className="course-list">{courses.filter(c=>c.date===selectedDay).map(c=><CourseCard key={c.id} c={c} onClick={()=>setSelected(c)}/>)}</div>:<div className="day-empty">当天暂无课程</div>}</Panel><Legend/></section>}
...
function Pager({prev,current,next}){return <div className="pager"><button aria-label="上一页" onClick={prev}><ChevronLeft/></button><button onClick={current}>当前</button><button aria-label="下一页" onClick={next}><ChevronRight/></button></div>}
