...
function StudentApp(){
  ...
  const nav=[{id:"today",label:"今日",icon:Home},{id:"week",label:"周课表",icon:CalendarDays},{id:"month",label:"月视图",icon:CalendarRange},{id:"search",label:"搜索",icon:Search},{id:"profile",label:"我的",icon:UserRound}];
  ...
  return <div className="shell">
    ...
    <nav className="bottom">{nav.map(n=><button key={n.id} className={tab===n.id?"active":""} onClick={()=>setTab(n.id)}><n.icon size={20}/><span>{n.label}</span></button>)}</nav>
...
function Pager({prev,next,anchor,courses,onSelectDate}){const [open,setOpen]=useState(false);const [pickerMonth,setPickerMonth]=useState(firstMonth(anchor));const anchorKey=iso(anchor);useEffect(()=>{if(open)setPickerMonth(firstMonth(anchor));},[open,anchorKey]);const start=monday(firstMonth(pickerMonth)),days=Array.from({length:42},(_,i)=>addDays(start,i)),courseDates=new Set(courses.map(c=>c.date));return <div className="pager"><button aria-label="上一页" onClick={prev}><ChevronLeft/></button><button className="current-picker" aria-expanded={open} onClick={()=>setOpen(value=>!value)}><CalendarDays size={17}/>选日期</button><button aria-label="下一页" onClick={next}><ChevronRight/></button>{open&&<div className="date-popover"><div className="date-popover-head"><button aria-label="上个月" onClick={()=>setPickerMonth(addMonths(pickerMonth,-1))}><ChevronLeft/></button><strong>{pickerMonth.getFullYear()}年{pickerMonth.getMonth()+1}月</strong><button aria-label="下个月" onClick={()=>setPickerMonth(addMonths(pickerMonth,1))}><ChevronRight/></button></div><div className="mini-weekdays">{["一","二","三","四","五","六","日"].map(x=><span key={x}>{x}</span>)}</div><div className="mini-calendar">{days.map(day=>{const key=iso(day);return <button key={key} className={`${day.getMonth()!==pickerMonth.getMonth()?"outside":""} ${key===anchorKey?"selected":""} ${key===iso(new Date())?"today":""}`} onClick={()=>{onSelectDate(day);setOpen(false)}}><span>{day.getDate()}</span>{courseDates.has(key)&&<i/>}</button>})}</div><button className="date-today" onClick={()=>{const date=new Date();onSelectDate(date);setOpen(false)}}>回到今天</button></div>}</div>}
