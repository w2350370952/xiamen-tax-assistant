...
function Pager({prev,next,onWeek,onMonth,view}){const [open,setOpen]=useState(false);return <div className="pager"><button aria-label="上一页" onClick={prev}><ChevronLeft/></button><button className="current-picker" aria-expanded={open} onClick={()=>setOpen(value=>!value)}>当前</button><button aria-label="下一页" onClick={next}><ChevronRight/></button>{open&&<div className="view-menu"><button className={view==="week"?"active":""} onClick={()=>{onWeek();setOpen(false)}}><CalendarDays size={17}/><span><b>当前周</b><small>查看周课表</small></span></button><button className={view==="month"?"active":""} onClick={()=>{onMonth();setOpen(false)}}><CalendarRange size={17}/><span><b>当前月</b><small>查看月课表</small></span></button></div>}</div>}
