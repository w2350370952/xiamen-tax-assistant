...
function MenuPage({menu,version,date,setDate,meal,setMeal,beijing,automaticMeal,resetCurrent}){const selectedDate=dateOf(date);const day=menu?.days?.find(item=>item.date===date);const categories=day?.meals?.[meal]||{};const itemCount=Object.values(categories).reduce((total,items)=>total+items.length,0);const mealMeta={breakfast:{icon:Coffee,note:"00:00–09:00"},lunch:{icon:Soup,note:"09:00–14:00"},dinner:{icon:MoonStar,note:"14:00–24:00"}};const go=count=>setDate(iso(addDays(selectedDate,count)));return <section className="page menu-page"><Header eyebrow="TODAY'S MENU" title="今天吃什么？" sub={`北京时间 ${beijing.time} · ${date===beijing.date?"今日菜单":`${dateOf(date).getMonth()+1}月${dateOf(date).getDate()}日菜单`}`} actions={<div className="menu-date-nav"><button aria-label="前一天" onClick={()=>go(-1)}><ChevronLeft/></button><button onClick={resetCurrent}><CalendarDays/>{date===beijing.date?"今天":date.slice(5).replace("-","/")}</button><button aria-label="后一天" onClick={()=>go(1)}><ChevronRight/></button></div>}/><div className="menu-banner"><div><span><UtensilsCrossed/></span><div><small>{day?.weekday||["星期日","星期一","星期二","星期三","星期四","星期五","星期六"][selectedDate.getDay()]}</small><h2>{date.replaceAll("-",".")}</h2><p>{date===beijing.date?`${mealLabels[automaticMeal]}时段 · 系统已按北京时间自动定位`:"使用左右箭头查看前后日期"}</p></div></div><aside><small>菜单版本</small><strong>{version?.label||"—"}</strong></aside></div><div className="meal-tabs">{Object.entries(mealLabels).map(([key,label])=>{const MetaIcon=mealMeta[key].icon;return <button key={key} className={meal===key?"active":""} onClick={()=>setMeal(key)}><MetaIcon/><span><strong>{label}</strong><small>{mealMeta[key].note}</small></span>{date===beijing.date&&key===automaticMeal&&<em>当前</em>}</button>})}</div>{day&&itemCount?<div className="menu-sheet"><div className="menu-sheet-head"><div><small>{mealLabels[meal].toUpperCase()} MENU</small><h2>{day.weekday} · {mealLabels[meal]}</h2></div><span>{itemCount} 道</span></div><div className="menu-category-list">{Object.entries(categories).map(([category,items],index)=>items.length?<article key={category} className={index%2?"alternate":""}><div><i>{String(index+1).padStart(2,"0")}</i><strong>{category}</strong></div><ul>{items.map((item,itemIndex)=><li key={`${item}-${itemIndex}`}>{item}</li>)}</ul></article>:null)}</div><footer>菜单以食堂当天实际供应为准</footer></div>:<div className="menu-empty"><Soup/><h2>当天暂无{mealLabels[meal]}菜单</h2><p>{menu?"可以切换其他餐次，或使用左右箭头查看其他日期。":"管理员发布一周菜单后，这里会自动显示每日菜品。"}</p></div>}</section>}
