...
function DishDictionary({ dictionary }) {
  ...
  const visible = dictionary.filter((entry) => (meal === "all" || entry.meal === meal) && (!keyword || normalizedDish(entry.name).includes(keyword) || (entry.aliases || []).some((alias) => normalizedDish(alias).includes(keyword))));
  const grouped = Object.entries(MENU_LABELS).map(([mealKey, label]) => ({ mealKey, label, entries: visible.filter((entry) => entry.meal === mealKey) })).filter((group) => group.entries.length);
  return <section className="dish-dictionary"><header><div><small>DISH MEMORY</small><h2>历史菜品库</h2><p>确认发布后的菜名会保存在这里，用于下次自动纠错和输入联想。炖汤、饮品即使本周没有，也不会从历史库删除。</p></div><label>搜索菜品<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="菜名或曾经的错别字"/></label></header><nav><button className={meal === "all" ? "active" : ""} onClick={() => setMeal("all")}>全部 <span>{dictionary.length}</span></button>{Object.entries(MENU_LABELS).map(([key, label]) => <button key={key} className={meal === key ? "active" : ""} onClick={() => setMeal(key)}>{label} <span>{dictionary.filter((entry) => entry.meal === key).length}</span></button>)}</nav>{grouped.length ? <div className="dish-dictionary-groups">{grouped.map((group) => <article key={group.mealKey}><h3>{group.label}<span>{group.entries.length} 道</span></h3><div>{group.entries.map((entry) => <section key={`${entry.meal}-${entry.category}-${entry.name}`}><small>{entry.category}</small><strong>{entry.name}</strong><p>使用 {entry.uses || 1} 次{entry.aliases?.length ? ` · 已记住 ${entry.aliases.length} 个识别别名` : ""}</p>{entry.aliases?.length > 0 && <em>曾识别为：{entry.aliases.join("、")}</em>}</section>)}</div></article>)}</div> : <div className="dish-dictionary-empty"><Soup/><h3>没有找到符合条件的菜品</h3><p>{dictionary.length ? "请更换搜索词或餐次" : "首次确认发布菜单后，系统会开始积累历史菜品。"}</p></div>}</section>;
}
