...
export default function AdminApp() {
  ...
  return <div className="admin-shell">
    <header className="admin-topbar"><a href="/"><ArrowLeft size={18}/>返回学生端</a><div><ShieldCheck/><span><strong>厦国会生活助手</strong><small>管理员控制台 · 腾讯云 EdgeOne</small></span></div><button onClick={logout}><LogOut size={17}/>退出登录</button></header>
...
function UserManagementPanel() {
  ...
  const load = async () => {
    setLoading(true); setError("");
    try {
      const [deviceData, dashboardData, regionData] = await Promise.all([api("/api/admin/devices"), api(`/api/admin/user-analytics?days=${range}`), api("/api/admin/user-regions")]);
      setDevices(deviceData.devices || []); setDashboard(dashboardData); setRegions(regionData);
    } catch (loadError) { setError(loadError.message); }
    finally { setLoading(false); }
  };
  useEffect(() => { load(); }, [range]);
  ...
  const keyword = query.trim().toLowerCase();
  const visible = devices.filter((item) => !keyword || [item.device_id, item.device_name, item.device_type, item.system, item.browser, item.fingerprint_id, item.ip_province, item.ip_city, item.ip_isp, item.remark].some((value) => String(value || "").toLowerCase().includes(keyword)));
  const locationText = (item) => [item.ip_province, item.ip_city].filter(Boolean).join(" ") || "未知";
  const maxRegion = Math.max(1, ...(regions?.provinces || []).map((item) => item.count), ...(regions?.cities || []).map((item) => item.count), ...(regions?.isps || []).map((item) => item.count));
  ...
  return <section className="user-management">
    ...
    <div className="behavior-dashboard region-dashboard"><RankingCard title="省份分布" eyebrow="PROVINCES" items={regions?.provinces || []} max={maxRegion}/><RankingCard title="城市分布" eyebrow="CITIES" items={regions?.cities || []} max={maxRegion}/><RankingCard title="运营商分布" eyebrow="ISP" items={regions?.isps || []} max={maxRegion}/></div>
    <div className="device-table-card"><header><div><small>DEVICE DIRECTORY</small><h3>设备列表</h3><p>共 {devices.length} 台设备，点击任意设备查看完整行为轨迹。</p></div><label>搜索设备<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="设备码、系统、浏览器或备注"/></label></header>{visible.length ? <><div className="device-table"><div className="device-table-head"><span>设备编号</span><span>设备类型</span><span>系统</span><span>浏览器</span><span>浏览器指纹</span><span>IP归属地</span><span>运营商</span><span>首次访问</span><span>最近访问</span><span>访问次数</span><span>操作</span></div>{visible.map((device) => <article key={device.device_id}><button className="device-code" onClick={() => openDetails(device.device_id)}><i>{deviceIcon(device)}</i><span><strong>{device.device_id}{device.suspected_same_user && <em className="suspect-badge">疑似同一用户</em>}</strong><small>{device.remark || device.device_name}</small></span></button><span>{device.device_type}</span><span>{device.system}</span><span>{device.browser}</span><span className="fp-cell">{device.fingerprint_id || "—"}</span><span>{locationText(device)}</span><span>{device.ip_isp || "未知"}</span><time>{beijingTime(device.first_visit_time)}</time><time>{beijingTime(device.last_visit_time)}</time><b>{device.visit_count}</b><div><button onClick={() => openDetails(device.device_id)}>查看详情</button><button className="delete" onClick={() => deleteDevice(device)}>删除</button></div></article>)}</div><div className="device-mobile-list">{visible.map((device) => <article key={device.device_id}><button onClick={() => openDetails(device.device_id)}><i>{deviceIcon(device)}</i><span><strong>{device.device_id}{device.suspected_same_user && <em className="suspect-badge">疑似同一用户</em>}</strong><small>{device.device_name} · {device.system} · {device.browser} · {locationText(device)}</small><em>{device.remark || `最近 ${beijingTime(device.last_visit_time, true)}`}</em></span><b>{device.visit_count}次</b></button></article>)}</div></> : <div className="user-empty"><Users/><h3>{devices.length ? "没有匹配的设备" : "还没有设备记录"}</h3><p>{devices.length ? "请更换搜索关键词" : "学生端产生新的访问后会自动出现在这里。"}</p></div>}</div>
    {details && <div className="device-detail-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setDetails(null)}><section className="device-detail"><button className="editor-close" onClick={() => setDetails(null)}><X/></button><header><span>{deviceIcon(details.device)}</span><div><small>DEVICE PROFILE</small><h2>{details.device.device_id}</h2><p>{details.device.device_name} · {details.device.system} · {details.device.browser}</p></div></header><div className="device-basic-grid"><article><small>设备类型</small><strong>{details.device.device_type}</strong></article><article><small>屏幕尺寸</small><strong>{details.device.screen_size || "未获取"}</strong></article><article><small>首次访问</small><strong>{beijingTime(details.device.first_visit_time)}</strong></article><article><small>最近访问</small><strong>{beijingTime(details.device.last_visit_time)}</strong></article></div><div className="device-info-block"><h4>设备信息</h4><div className="device-basic-grid"><article><small>浏览器指纹</small><strong>{details.device.fingerprint_id || "未采集"}</strong></article><article><small>浏览器版本</small><strong>{details.device.browser_version || "未知"}</strong></article><article><small>语言</small><strong>{details.device.language || "未知"}</strong></article><article><small>时区</small><strong>{details.device.timezone || "未知"}</strong></article></div></div><div className="device-info-block"><h4>网络信息</h4><div className="device-basic-grid"><article><small>IP地址</small><strong>{details.device.ip_address || "未记录"}</strong></article><article><small>IP归属地</small><strong>{locationText(details.device)}</strong></article><article><small>运营商</small><strong>{details.device.ip_isp || "未知"}</strong></article><article><small>国家/地区</small><strong>{details.device.ip_country || "未知"}</strong></article></div></div>
    {details.related_devices?.length > 0 && <div className="device-info-block"><h4>关联设备（指纹相同，疑似同一用户）</h4><div className="related-device-list">{details.related_devices.map((item) => <article key={item.device_id}><button className="device-code" onClick={() => openDetails(item.device_id)}><i>{deviceIcon(item)}</i><span><strong>{item.device_id}</strong><small>{item.device_name} · {item.system} · {item.browser}{item.ip_city ? ` · ${item.ip_city}` : ""}</small></span></button><time>{beijingTime(item.last_visit_time)}</time></article>)}</div></div>}<div className="device-stat-grid"><article><small>累计访问</small><strong>{details.device.visit_count}<em>次</em></strong></article><article><small>行为记录</small><strong>{details.statistics.action_count}<em>条</em></strong></article><article><small>最常访问页面</small><strong>{details.statistics.favorite_page}</strong></article><article><small>最常操作</small><strong>{details.statistics.favorite_action}</strong></article></div><div className="device-remark"><label>管理员备注<input value={remark} onChange={(event) => setRemark(event.target.value)} maxLength={200} placeholder="例如：经常查看菜单的 iPhone"/></label><button disabled={loading} onClick={saveRemark}>保存备注</button></div><div className="device-log"><header><h3>历史访问记录</h3><span>最近 {details.logs.length} 条</span></header>{details.logs.length ? <div><div className="device-log-head"><span>时间</span><span>页面</span><span>操作</span></div>{details.logs.map((log) => <article key={log.id}><time>{beijingTime(log.create_time)}</time><strong>{log.page_name}</strong><p><b>{log.action_type}</b>{log.action_detail && <span>{log.action_detail}</span>}</p></article>)}</div> : <p className="no-log">暂无行为记录</p>}</div><button className="delete-device" onClick={() => deleteDevice(details.device)}><Trash2/>删除该设备及全部记录</button></section></div>}
  </section>;
}
