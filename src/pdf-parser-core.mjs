...
function columnText(items, minX, maxX) {
  return items.filter(item => item.x >= minX && item.x < maxX && clean(item.str))
    .sort((a, b) => Math.abs(b.y - a.y) > 1 ? b.y - a.y : a.x - b.x)
    .map(item => clean(item.str)).join("");
}

function extractPeriodsFromText(items) {
  const normalized = items.map(item => clean(item.str)).join("").replaceAll("：", ":").replace(/[—–]/g, "-");
  const result = { 上午: ["08:30", "11:30"], 下午: ["14:30", "17:30"], 晚上: ["18:30", "21:30"] };
...
export function parseExtractedPages(pages) {
  const warnings = [], courses = [], seen = new Set();
  const periods = extractPeriodsFromText(pages.flatMap(page => page.items));
  for (const page of pages) {
    const items = page.items.filter(item => clean(item.str));
    const dates = items.filter(item => item.x < 130 && DATE_RE.test(clean(item.str))).sort((a, b) => b.y - a.y);
    if (!dates.length) { warnings.push(`第 ${page.pageNumber} 页未识别到日期行，请人工复核`); continue; }
    dates.forEach((dateItem, index) => {
      ...
      const periodItems = block.filter(item => item.x >= 170 && item.x < 210 && ["上午", "下午", "晚上"].includes(clean(item.str))).sort((a, b) => b.y - a.y);
      periodItems.forEach(periodItem => {
        ...
        const assigned = block.filter(item => {
          ...
          const closest = periodItems.reduce((best, candidate) => Math.abs(candidate.y - item.y) < Math.abs(best.y - item.y) ? candidate : best, periodItems[0]);
        ...
        const course = {
          date, weekday, period, start_time: startTime, end_time: endTime,
          course_name: name, teacher: columnText(assigned, 350, 400),
          class_name: columnText(assigned, 400, 445), classroom: columnText(assigned, 445, 490),
          remark: columnText(assigned, 490, 560), course_type: courseType(name), source_page: page.pageNumber,
        };
